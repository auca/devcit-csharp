...
## Задание №5: Случайное число

Создайте C# файл `Utilities.cs` с методом `double randomDouble(min, max)`, которая может
генерировать целые числа в указанном диапазоне (от `min` до `max`).

## Задание №6: Анимация

Cоздайте симуляцию движения молекул в замкнутом пространстве.
